        ...
        try {
            ...
            let users = contacts.map(async (contact) => {
                if (contact.contactId == currentUserId) {
                    return await UserModel.getNomalUserDataById(contact.userId);
                } else {
                    return await UserModel.getNomalUserDataById(contact.contactId);
                }                
            });
...
/**
 * Read more contacts received, max 10 items one time
 * @param {string} currentUserId 
 * @param {number} skipNumberContacts 
 */
