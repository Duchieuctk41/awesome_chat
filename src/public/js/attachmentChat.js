function attachmentChat(divId) {
    $(`#attachment-chat-${divId}`).unbind("change").on("change", function() {
        ...
        if(fileData.size > limit) {
            alertify.notify("Tệp tin đính kèm tối đa cho phép là 1MB", "error", 7);
            $(this).val(null);
            return false;
        }
        ...
        $.ajax({
            ...
            if (isChatGroup) {
             ...
             $(`.person[data-chat=${divId}]`).find("span.preview").removeClass("message-time-realtime").html("Tệp đính kèm...");
...
$(document).ready(function() {
    socket.on("response-chat-attachment", function(response) {
        ...
        $(`.person[data-chat=${divId}]`).find("span.preview").addClass("message-time-realtime").html("Tệp đính kèm...");
